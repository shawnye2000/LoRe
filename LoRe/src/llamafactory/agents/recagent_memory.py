...
class ShortTermMemory():
    ...
    def __init__(self, llm, embedding_model):
        ...
        _,self.embeddings_model= None, embedding_model #utils.get_embedding_model()
...
class RecAgentMemory(BaseMemory):
    ...
    def __init__(self,llm, memory_retriever, now, embedding_model, verbose=False, reflection_threshold=None, ):
        super(RecAgentMemory, self).__init__()

        # self.llm = llm
        self.now = now
        self.sensoryMemory = SensoryMemory(llm)
        self.shortTermMemory = ShortTermMemory(llm, embedding_model = embedding_model)
        self.longTermMemory = LongTermMemory(llm=llm, memory_retriever=memory_retriever,
                                             now=self.now, verbose=verbose,
