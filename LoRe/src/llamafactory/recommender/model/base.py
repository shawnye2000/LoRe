...
class BaseModel(object):
    ...
    def __init__(self, config, input_data=None, device=None):
        super().__init__()
        self.config = config
        self.items = None
        # self.wksp = init_para['workspace']
        # self.name = init_para['name']
        self.device = device  #init_para['device']
        # self.patience = init_para['patience']  # patience for early stop
        # self.loss_func = init_para['loss_function']
        self.input_data_ls = input_data
        if config['rec_model'] == 'DIN' or config['rec_model'] == 'Signal':
    ...
    def embed_input_fields_for_attention_pooling(self, input_data):
        ...
        for data_filed, data in zip(self.input_data_ls, input_data):
            if data_filed.name.startswith('history'):
                ...
                # print(f'emb_tab_dict:{self.emb_tab_dict}')
                history_data = self.emb_tab_dict[f'{data_filed.name[len("history") + 1:]}_emb_tab'](data)
                if len(data.size()) == 2:
                    pass
                elif len(data.size()) == 3:
                ...
                if data_filed.name in ['item_cate_id']:
                    ...
                    # print(f'data_filed.name{data_filed.name}')
                    # print(f'data:{data}')
                    history_data = self.emb_tab_dict[f'{data_filed.name}_emb_tab'](data)
        ...
        # print('item')
        # for ls in item_emb_ls:
        #     print(ls.shape)
        #
        # print('history')
        # for ls in history_emb_ls:
        #     print(ls.shape)

        # print(f'history emb ls:{history_emb_ls}')
    ...
    def embed_orig_and_aug_data_wocot_fileds(self, x):
        ...
        final_aug_data_emb = [[self.history_mlp(torch.cat(data[:-1], dim=-1))] + data[-1:] for data in
                              aug_data_emb_wocot]
    ...
    def set_emb_tab_for_aug_data(self, item_text_emb_dict: dict):

        '''BGE embeddings of item textual feature'''
        item_text_emb = torch.zeros((len(item_text_emb_dict) + 1, 768))  # zero for padding
        for item_id, text_emb in item_text_emb_dict.items():  # id: text_emb
